...
export function WorkbenchTaskDraftSection({
  ...
  const collapsedDuringActiveRunRef = useRef<{
  ...
  useEffect(() => {
    if (shouldCollapseAuxiliaryUi && !previousActiveRunRef.current) {
      collapsedDuringActiveRunRef.current = {
        showAdvanced,
        showWorkspaceConfig,
      };
    ...
    if (!shouldCollapseAuxiliaryUi && previousActiveRunRef.current) {
      if (collapsedDuringActiveRunRef.current?.showAdvanced) {
        setShowAdvanced(true);
      }
      if (collapsedDuringActiveRunRef.current?.showWorkspaceConfig) {
        setShowWorkspaceConfig(true);
      }
      collapsedDuringActiveRunRef.current = null;
    }

    previousActiveRunRef.current = shouldCollapseAuxiliaryUi;
  }, [shouldCollapseAuxiliaryUi, showAdvanced, showWorkspaceConfig]);
  ...
  const showRuntimeRow = pendingApprovalActive || !shouldCollapseAuxiliaryUi;
  ...
  return (
    <View style={screenStyles.composerBar}>
      ...
      {!shouldCollapseAuxiliaryUi && showWorkspacePanel ? (
        <View
          style={[
