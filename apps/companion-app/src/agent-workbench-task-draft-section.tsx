...
export function WorkbenchTaskDraftSection({
  ...
  const showIntentResolverHint =
    ...
    draftTask === null;
  const resolvedTaskUsesAdvancedCommand = draftCommand.trim().length > 0;
  const showResolvedTaskSummary = draftTask !== null && !showAdvanced;
  const resolvedTaskSummaryDetail =
    draftTask === null
      ? null
      : resolvedTaskUsesAdvancedCommand
        ? appI18n.agentWorkbench.taskDraft.resolvedAdvancedCommandDetail(
            currentWorkspaceLabel,
          )
        : appI18n.agentWorkbench.taskDraft.resolvedStarterTaskDetail(
            draftTask.goal,
            currentWorkspaceLabel,
          );
  const resolvedTaskSourceLabel = resolvedTaskUsesAdvancedCommand
    ? appI18n.agentWorkbench.taskDraft.resolvedAdvancedCommandBadge
    : appI18n.agentWorkbench.taskDraft.resolvedStarterTaskBadge;
  const resolvedTaskSourceIcon = resolvedTaskUsesAdvancedCommand
    ? iconCatalog.code
    : iconCatalog.terminal;
  const resolvedTaskSourceColor = resolvedTaskUsesAdvancedCommand
    ? palette.inkSoft
    : palette.accent;
  ...
  return (
    <View style={screenStyles.composerBar}>
      {showIntentResolverHint ? (
        <InfoPanel
          testID='agent-workbench.task.unresolved-hint'
          title={appI18n.agentWorkbench.taskDraft.intentResolverHintTitle}
          tone='accent'>
          <Text
            testID='agent-workbench.task.unresolved-hint.detail'
            style={[
      ...
      {showResolvedTaskSummary && resolvedTaskSummaryDetail ? (
        <View
          testID='agent-workbench.task.resolved-summary'
          style={[
            screenStyles.workspaceSetupCard,
            screenStyles.composerResolvedTaskCard,
            {
              borderColor: palette.border,
              backgroundColor: palette.panel,
            },
          ]}>
          <View style={screenStyles.composerResolvedTaskHeader}>
            <Text
              testID='agent-workbench.task.resolved-summary.title'
              style={[screenStyles.inputLabel, {color: palette.inkSoft}]}>
              {appI18n.agentWorkbench.taskDraft.resolvedTaskSummaryTitle}
            </Text>

            <View
              style={[
                screenStyles.composerChip,
                screenStyles.composerResolvedTaskSourceChip,
                {
                  backgroundColor: palette.canvasShade,
                  borderColor: resolvedTaskUsesAdvancedCommand
                    ? palette.border
                    : palette.accent,
                },
              ]}>
              <Icon
                icon={resolvedTaskSourceIcon}
                size={12}
                color={resolvedTaskSourceColor}
              />
              <Text
                testID='agent-workbench.task.resolved-summary.source'
                style={[
                  screenStyles.composerChipLabel,
                  {color: resolvedTaskSourceColor},
                ]}>
                {resolvedTaskSourceLabel}
              </Text>
            </View>
          </View>

          <Text
            testID='agent-workbench.task.resolved-summary.detail'
            style={[
              screenStyles.sectionDescription,
              {color: palette.inkMuted},
            ]}>
            {resolvedTaskSummaryDetail}
          </Text>

          <View
            style={[
              screenStyles.terminalBox,
              screenStyles.composerResolvedTaskCommandBox,
              {backgroundColor: palette.canvasShade},
            ]}>
            <Text
              testID='agent-workbench.task.resolved-summary.command'
              style={[
                screenStyles.terminalText,
                screenStyles.composerResolvedTaskCommandText,
                {color: palette.ink},
              ]}>
              {draftTask.command}
            </Text>
