export const zhCNApp = {
  ...
  agentWorkbench: {
    ...
    approval: {
      pendingTitle: '待处理审批',
      promptTitle: '这条命令需要你的授权后才能继续。',
      reasonLabel: '申请审批原因',
      commandLabel: '审批执行的命令内容',
      actionsLabel: '审批动作',
      approveOnceOption: '批准本次',
      approveOnceOptionDetail: '只放行这一次命令。',
      approvePrefixOption: '批准此次及后续相同动作',
      approvePrefixOptionDetail: '相同命令前缀、cwd 和权限模式会自动放行。',
      rejectOption: '拒绝并输入原因',
      rejectOptionDetail: '把拒绝原因回给模型，让它继续收尾。',
      rejectReasonLabel: '拒绝原因',
      rejectReasonPlaceholder: '说明为什么这次不能执行这条命令',
      submitAction: '提交审批决定',
      commandWillRunAfterApproval: '批准后才会执行',
      commandApprovedHint: '已获批准并继续执行',
      commandRejectedHint: '已被拒绝，不会执行',
      commandExpiredHint: '已过期，不会执行',
      diffPreviewBadge: '会生成 diff 预览',
    ...
    timeline: {
      ...
      rejectedCommand: (command: string) => `已拒绝 ${command}`,
      runningCommandCard: '运行命令中',
      completedCommandCard: '已运行命令',
      failedCommandCard: '命令失败',
      cancelledCommandCard: '已取消命令',
      rejectedCommandCard: '授权已拒绝',
      rejectedCommandOutput: '授权被拒绝，命令未执行。',
