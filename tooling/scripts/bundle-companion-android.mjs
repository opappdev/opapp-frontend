...
const outputRoot = path.join(repoRoot, '.dist', 'bundles', 'companion-app', 'android');
const bundleOutput = path.join(outputRoot, 'index.android.bundle');
...
async function main() {
  ...
  const manifest = {
    ...
    platform: 'android',
    // Android host's getBundleAssetName() expects the full filename including .bundle extension.
  ...
  console.log(
    `Bundled companion app for Android: ${bundleOutput}${resetCache ? ' (with reset-cache)' : ''}`,
  );
